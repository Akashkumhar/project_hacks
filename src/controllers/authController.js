...
import { addPendingSignup, getPendingSignup, removePendingSignup } from '../utils/pendingSignups.js';
...
export const signup = async (req, res) => {
  try {
    ...
    addPendingSignup({ userName, email, password: hashedPassword, otp, otpExpiry });
...
export const verifyOtp = async (req, res) => {
  try {
    ...
    if (!pending) {
      return res.status(400).json({ message: 'No pending signup found or OTP expired.' });
    }
    if (pending.otp !== otp || !pending.otpExpiry || pending.otpExpiry < new Date()) {
      return res.status(400).json({ message: 'Invalid or expired OTP' });
    }
    
    const user = new User({ userName: pending.userName, email: pending.email, password: pending.password, isVerified: true });
    await user.save();
